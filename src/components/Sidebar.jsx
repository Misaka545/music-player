...
const Sidebar = ({ libraryAlbums, onScanFolder, onScanFiles, onViewChange, onAlbumSelect }) => {
    ...
    const confirmCreatePlaylist = () => {
        if (newPlaylistName.trim() !== "") { createPlaylist(newPlaylistName); setFilterMode('playlists'); setIsModalOpen(false); showToast(`PLAYLIST_CREATED // ${newPlaylistName.trim()}`, 'SUCCESS'); }
    };
    ...
    return (
        <>
            <div className="w-64 flex flex-col gap-3 h-full hidden md:flex relative">
                {/* LOGO AREA */}
                <div onClick={() => setIsManualModalOpen(true)} className="h-16 bg-[#111] border border-[#333] flex flex-col justify-center relative overflow-hidden group cursor-pointer hover:bg-[#1a1a1a] transition-colors">
            ...
            >
                <div className="space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
                    <div>
                        <h3 className="text-[#FF6B35] font-bold text-[10px] tracking-widest uppercase mb-3 border-b border-[#333] pb-1">Keyboard Shortcuts</h3>
                        <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-xs font-mono">
                            <div className="text-[#999] text-right">Space</div><div className="text-white">Play / Pause</div>
                            <div className="text-[#999] text-right">← / →</div><div className="text-white">Previous / Next Track</div>
                            <div className="text-[#999] text-right">Ctrl + S</div><div className="text-white">Toggle Shuffle</div>
                            <div className="text-[#999] text-right">Ctrl + R</div><div className="text-white">Cycle Repeat Mode</div>
                            <div className="text-[#999] text-right">M</div><div className="text-white">Mute / Unmute</div>
                            <div className="text-[#999] text-right">K / J</div><div className="text-white">Volume Up / Down</div>
                            <div className="text-[#999] text-right">F</div><div className="text-white">Toggle Fullscreen</div>
                            <div className="text-[#999] text-right">Escape</div><div className="text-white">Close Menus / Go Back</div>
                        </div>
                    </div>
                    <div>
                        <h3 className="text-[#4FD6BE] font-bold text-[10px] tracking-widest uppercase mb-3 border-b border-[#333] pb-1">Features</h3>
                        <ul className="text-xs text-[#ccc] space-y-2 list-disc pl-4">
                            <li><strong>Exclusive Mode (WASAPI):</strong> Bypasses the OS mixer for bit-perfect audio delivery to your DAC.</li>
                            <li><strong>Background Play:</strong> Close the app window to minimize it to the system tray while keeping the music playing.</li>
                            <li><strong>Database:</strong> Scan your local folders or files. Metadata and covers are automatically extracted and stored.</li>
                        </ul>
                    </div>
                    <div className="pt-3 border-t border-[#333] text-[10px] text-[#555] text-center font-mono">
                        [PROJECT_LT] RL09 - TERMINAL_V1<br />
                        Retrograde Audio Player
                    </div>
                </div>
            </CustomModal>
        </>
